...
class ResultCallback(CallbackBase):
    def v2_runner_on_ok(self, result, **kwargs): # result 包含 '_check_key', '_host', '_result', '_task', '_task_fields', 'is_changed', 'is_failed', 'is_skipped', 'is_unreachable', 'task_name'
      host = result._host
      print("%s 执行结果" % result._task)
...
# 用来加载解析yaml文件或JSON内容,并且支持vault的解密 # 定义选项
Options = namedtuple('Options', ['connection', 'module_path', 'forks', 'become', 'become_method', 'become_user', 'check'])

# 定义连接远端的方式为smart
options = Options(connection='smart', module_path=None, forks=5, become=None, become_method=None, become_user=None, check=False)
...
play_source = dict(
    name = "Ansible Play",
    hosts = 'all', # 匹配host_list中的主机的正则表达式
    gather_facts = 'no',
    # 定义任务列表
    tasks = [
...
try:
    ...
    result = tqm.run(play)
    print('任务执行返回码: %s' % result)
    # 返回码，只要有一个 host 出错就会返回 非0 数字
finally:
    if tqm is not None:
        tqm.cleanup()
    if loader:
...
# http://www.jianshu.com/p/62b6d2325648